...
def train_point(M, epochs, N=80, p=40, hidden=96, n_train=512, seed=0):
    ...
    t0 = time.time()
    hist = op.train(
        Str, Atr, epochs=epochs, lr=3e-3, batch=96, val=(Sval, Aval), log_every=max(1, epochs // 4)
    )
    secs = time.time() - t0
    ...
    print(
        "M=%d  input_dim=%d  rel-L2 mean=%.3f median=%.3f p90=%.3f  (%.0fs, %d ep)"
        % (
            M,
            row["n_params"],
            row["rel_l2_mean"],
...
def plot():
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig("results/exp15_highdim.png", dpi=140)
    plt.close(fig)
    print(
        "wrote results/exp15_highdim.png from",
        len(data),
        "points:",
