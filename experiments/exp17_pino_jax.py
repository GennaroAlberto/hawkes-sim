...
from hawkes_calibration.operators.pino import sample_instances, exact_solution
...
def run(M, out_dir="results", seed=0):
    ...
    print(f"=== exp17 PINO (JAX), M={M}  net[h={c['hidden']},p={c['p']}]  "
          f"data[train={c['n_train']},anchor={c['n_anchor']}]  epochs={c['epochs']} ===", flush=True)
    op = JAXMultivariateMBPPOperator(M, t, theta=1.0, p=c["p"], hidden=c["hidden"],
                                     depth=c["depth"], n_fourier=16, seed=0)
    t0 = time.time()
    hist = op.train(Str, Atr, epochs=c["epochs"], lr=2e-3, batch=256,
                    anchors=(Sa, Aa, XIa), w_res=1.0, w_anchor=1.0, w_ic=1.0,
                    val=(Sval, Aval), log_every=max(1, c["epochs"] // 6))
    secs = time.time() - t0
    ...
    rel = num / den
    per_sector = [float(np.mean(np.linalg.norm(xi[:, :, m] - XI[:, :, m], axis=1)
                                / (np.linalg.norm(XI[:, :, m], axis=1) + 1e-12))) for m in range(M)]
    rho = np.array([float(np.max(np.abs(np.linalg.eigvals(a)))) for a in Atest])

    row = dict(M=M, input_dim=int(M + M * M), N=c["N"], epochs=c["epochs"], seconds=round(secs, 1),
               p=c["p"], hidden=c["hidden"], n_train=c["n_train"], n_anchor=c["n_anchor"],
               rel_l2_mean=float(rel.mean()), rel_l2_median=float(np.median(rel)),
               rel_l2_p90=float(np.percentile(rel, 90)), rel_l2_p99=float(np.percentile(rel, 99)),
    ...
    print(f"  M={M}: held-out rel-L2 mean={rel.mean():.4f} median={np.median(rel):.4f} "
          f"p90={np.percentile(rel,90):.4f}  ({secs:.0f}s)", flush=True)
    print(f"  per-sector={[round(x,4) for x in per_sector]}", flush=True)
    print(f"  by branching: low-rho={row['rel_low_rho']}  high-rho={row['rel_high_rho']}", flush=True)
    return row
...
def _plot_instance(out_dir, M, t, xi, XI, rel, rho):
    ...
    ax[0].set_xlabel("t"); ax[0].set_ylabel(r"$\xi_m(t)$")
    ...
    ax[1].set_title("held-out rel-L2 (400 unseen)"); ax[1].set_xlabel("rel-L2"); ax[1].legend()
    ax[2].scatter(rho, rel, s=12, alpha=0.5, color="C4")
    ax[2].set_title("accuracy vs branching ratio"); ax[2].set_xlabel(r"$\rho(A)$"); ax[2].set_ylabel("rel-L2")
    fig.tight_layout(); fig.savefig(os.path.join(out_dir, f"exp17_pino_jax_M{M}.png"), dpi=140); plt.close(fig)
...
def plot_summary():
    ...
    for r in data:
        ax.annotate(f"{r['M']}-d", (r["M"], r["rel_l2_mean"]), textcoords="offset points", xytext=(5, 6))
    ax.set_xlabel("sectors M (excitation matrix M×M)"); ax.set_ylabel("held-out rel-L2")
    ax.set_title("Multivariate MBPP PINO accuracy vs dimension (JAX, hybrid)")
    ax.grid(alpha=0.3); ax.legend()
    fig.tight_layout(); fig.savefig("results/exp17_pino_jax_summary.png", dpi=140); plt.close(fig)
