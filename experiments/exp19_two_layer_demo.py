...
from hawkes_calibration.sector_ranker import (
    StartupRankerResult, sector_baseline_rates, poisson_nll,
)

CFG = dict(T=170, n_sectors=8, startups_per_sector=25, n_covariates=2, n_lags=3,
           cooldown_weeks=16, base_log_rate=-1.0, covariate_strength=4.5,
           excitation_radius=0.10, train_end=120)


def _oracle(data, cooldown_weeks):
    p = data.true_ranker_weights.shape[1]
    return StartupRankerResult(global_weights=np.zeros(p),
                               sector_deviations=data.true_ranker_weights,
                               cooldown_coef=data.true_ranker_cooldown,
                               cooldown_weeks=cooldown_weeks, loss=0.0,
                               success=True, message="oracle")


def run(seeds=range(6), out_dir="results"):
    os.makedirs(out_dir, exist_ok=True)
    c = CFG
    sector_rows, fit_rows, orac_rows, extra = [], [], [], []
    for seed in seeds:
        d = simulate_synthetic_startup_market(
            T=c["T"], n_sectors=c["n_sectors"], startups_per_sector=c["startups_per_sector"],
            n_covariates=c["n_covariates"], n_lags=c["n_lags"], cooldown_weeks=c["cooldown_weeks"],
            base_log_rate=c["base_log_rate"], covariate_strength=c["covariate_strength"],
            excitation_radius=c["excitation_radius"], seed=seed)
        te = c["train_end"]

        # --- Layer 1: sector count model vs historical-mean baseline ---
        sf = fit_sector_count_model(d.sector_counts, d.covariates, n_lags=c["n_lags"],
                                    train_end=te, max_excitation_radius=0.3, l2=1e-2)
        rates = sf.rates(d.sector_counts, d.covariates)
        obs = d.sector_counts[te:c["T"]]
        model_nll = poisson_nll(obs, rates[te:c["T"]]) / obs.size
        base_nll = poisson_nll(obs, sector_baseline_rates(d.sector_counts, te, te, c["T"])) / obs.size
        sector_rows.append(dict(seed=int(seed), model_nll=model_nll, baseline_nll=base_nll,
                                improvement=base_nll - model_nll,
                                fitted_spectral_radius=excitation_spectral_radius(sf.excitation),
                                mean_count=float(obs.mean()), max_count=int(d.sector_counts.max())))

        # --- Layer 2: ranker vs random and vs oracle ---
        args = (d.events, d.startup_features, d.startup_sector, d.active, d.startup_counts)
        rf = fit_startup_ranker(*args, train_end=te, cooldown_weeks=c["cooldown_weeks"],
                                l2_global=1e-3, l2_sector=5e-2)
        fit_rows.append(evaluate_ranker(rf, *args, start_week=te, end_week=c["T"]))
        orac_rows.append(evaluate_ranker(_oracle(d, c["cooldown_weeks"]), *args, start_week=te, end_week=c["T"]))

    def avg(rows, k):
        return float(np.mean([r[k] for r in rows]))

    res = dict(
        config=c, n_seeds=len(list(seeds)),
        sector=dict(
            model_nll=avg(sector_rows, "model_nll"), baseline_nll=avg(sector_rows, "baseline_nll"),
        ...
        ranker=dict(
            ...
            random={k: avg(fit_rows, "random_" + k) for k in ("nll", "mrr", "top1", "top5", "top10")}),
        per_seed_sector=sector_rows)
    json.dump(res, open(os.path.join(out_dir, "exp19_two_layer.json"), "w"), indent=2, default=float)
...
def _print(res):
    ...
    print(f"  regime: ~{s['mean_count_per_cell']:.1f} events/sector-week, fitted sector "
    ...
    print(f"    model {s['model_nll']:.3f}  vs  baseline {s['baseline_nll']:.3f}   "
    ...
    for name in ("fitted", "oracle", "random"):
        d = r[name]
        print(f"    {name:7s} {d['top1']:.3f}   {d['top5']:.3f}   {d['top10']:.3f}   {d['mrr']:.3f}")
...
def _plot(out_dir, res, sector_rows):
    ...
    width = 0.38
    ax[0].bar([s - width / 2 for s in seeds], [r["model_nll"] for r in sector_rows], width, label="sector model", color="C0")
    ax[0].bar([s + width / 2 for s in seeds], [r["baseline_nll"] for r in sector_rows], width, label="mean baseline", color="C7")
    ax[0].set_xlabel("seed"); ax[0].set_ylabel("held-out Poisson NLL / cell")
    ax[0].set_title("Layer 1: sector count model beats baseline"); ax[0].legend()
    ...
    ax[1].set_xticks(x); ax[1].set_xticklabels(["top-1", "top-5", "top-10", "MRR"])
    ax[1].set_title("Layer 2: ranker at the oracle ceiling"); ax[1].legend()
    fig.suptitle("Two-layer marked model (no event-time info) -- both layers working")
    fig.tight_layout(); fig.savefig(os.path.join(out_dir, "exp19_two_layer.png"), dpi=140); plt.close(fig)
