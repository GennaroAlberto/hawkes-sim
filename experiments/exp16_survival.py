...
def simulate_population(n_firms=500, T=60.0, seed=0):
    ...
    events = []
    acq_time = np.where(
        rng.uniform(size=n_firms) < 0.25,  # 25% get acquired (competing risk)
        rng.uniform(5, T, n_firms),
        np.inf,
    )
    ...
    return dict(
        events=events, X_static=X_static, acq_time=acq_time, T=T, pop=pop, sector=sector, lin=lin
    )
...
class DiscreteHazard:
    ...
    def _targets(self, t, e):
        ...
        Y = np.zeros((len(t), self.K))
        Mk = np.zeros((len(t), self.K))
...
def _plot(out_dir, sim, X, t, e, te, grid, km, base, ranked, out):
    ...
    a.set_xlabel("time since decision (to next funding)")
    a.set_ylabel("S(t) = P(not yet funded)")
    a.legend(fontsize=7)

    a = ax[0, 1]
    names = ["popularity\nbaseline", "survival\nlikelihood", "survival\n+ranking"]
    cidx = [
        out["baseline_popularity"]["c_index"],
        out["likelihood"]["c_index"],
        out["likelihood+ranking"]["c_index"],
    ]
    a.bar(names, cidx, color=["0.6", "C0", "C2"])
    a.axhline(0.5, color="r", ls="--", label="random (0.5)")
    a.set_ylim(0.5, max(cidx) + 0.05)
    a.set_title("(b) concordance index (ranking quality)")
    a.legend()
    ...
    a.hist(
        ...
        stacked=True,
        color=["C2", "0.8"],
        label=["funded within horizon", "not / censored"],
    )
    a.set_title("(c) predicted funding risk, by actual outcome")
    a.set_xlabel("predicted P(funded within horizon)")
    a.set_ylabel("count")
    a.legend(fontsize=8)
    ...
    a.set_xlabel("watch-list fraction k")
    a.set_ylabel("recall (funders captured)")
    a.legend(fontsize=8)

    fig.suptitle(
        "Survival-with-ranking for next-funding prediction (KM + discrete-hazard DeepHit-style)"
    )
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, "exp16_survival.png"), dpi=140)
    plt.close(fig)
