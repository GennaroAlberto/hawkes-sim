...
def run(seeds=range(5), T=4000.0, out_dir="results"):
    ...
    for seed in seeds:
        ...
        events = simulate_multivariate_hawkes(gamma0, alpha, beta, T=T, gamma=gamma,
                                              covariate=cov, seed=int(rng.integers(1e9)))
    ...
    out = dict(M=M, p=p, T=T, n_seeds=len(list(seeds)), branching_radius=rho,
    ...
    json.dump(out, open(os.path.join(out_dir, "exp20_nonlinear_hawkes.json"), "w"), indent=2, default=float)

    print("=== Experiment 20: non-linear (exp-covariate) event-time Hawkes ===")
    print(f"  M={M}, {p} covariates, T={T:.0f}, ~{np.mean(n_ev):.0f} events; "
    ...
    print(f"    alpha  (excitation)      : {out['recovery']['alpha_max_abs_err']:.3f}  "
...
def _plot(out_dir, gamma0, gamma, alpha, beta, last):
    ...
    ax[0].scatter(gamma.ravel(), res.gamma.ravel(), s=40, color="C0", marker="s", label="gamma (covariate)")
    lim = [min(alpha.min(), gamma.min()) - 0.1, max(alpha.max(), gamma.max()) + 0.1]
    ax[0].plot(lim, lim, "k--", lw=1)
    ax[0].set_xlabel("true value"); ax[0].set_ylabel("MLE recovered")
    ax[0].set_title("Parameter recovery from event times"); ax[0].legend()
    ...
    ax[1].set_xlabel("t"); ax[1].set_ylabel(r"$\lambda_0$ baseline")
    ax[1].set_title("Nonlinear (exponential) covariate baseline, dim 0"); ax[1].legend()
    fig.suptitle("Non-linear-intensity Hawkes, recovered from event-time information")
    fig.tight_layout(); fig.savefig(os.path.join(out_dir, "exp20_nonlinear_hawkes.png"), dpi=140); plt.close(fig)
