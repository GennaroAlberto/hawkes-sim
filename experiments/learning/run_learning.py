...
def numpy_selfcheck():
    ...
    kernel = lambda tt, uu: (
        kappa
        * theta
        * np.exp(np.clip(delta * float(Zfn(tt)[0]), -30, 30))
        * np.exp(-theta * (tt - uu))
    )
    xi_lin = solve_mbpp_residual_linear(t, forcing, kernel)
    xi_vol = solve_mbpp_volterra(
        lambda tt: np.array([forcing(tt)]), lambda tt, uu: kernel(tt, uu), t, M=1
    )[:, 0]
    ...
    predict = lambda p, z: ref.solve(
        dict(kappa=p[0], theta=p[1], mu=p[2], delta=[p[3]], Z=_grid_callable(t, z)), t
    )
    ...
    print(
        "  linear-residual vs Volterra solve  : %.2e   (the residual's zero = exact solution)"
        % lin_err
    )
...
def _require(backend):
    ...
    try:
        ...
        raise SystemExit(
            "This experiment needs '%s' installed (pip install %s). [%s]"
...
def train_pinn(n_steps=20000, T=configs.T_DEFAULT):
    ...
    solver = make_neural_solver(
        backend="jax", mode="pinn", coll_grid=t, Z_on_grid=Z, T=T, n_delta=1
    )
    ...
    rep = solver_accuracy_report(
        ...
        t,
        configs.sample_params(64, seed=99),
        Z_array=None,
...
def train_pino(n_steps=30000, T=configs.T_DEFAULT, hybrid=True):
    ...
    if hybrid:
        ...
        kw = dict(
            anchors=(Z, P, make_anchor_data(t, P, Z)),
            data_weight=1.0,
            curriculum=True,
            reweight=True,
        )
    ...
    return solver_accuracy_report(
        ...
        P_te,
        Z_te,
        plot_path=configs.results_dir() + "/pino_accuracy.png",
    )
...
def train_amortized(epochs=60, M=10):
    ...
    net.fit(C, K, epochs=epochs, batch_size=128, verbose=0)
    return net
