...
def block_P(n_pinn=30000, n_pino=30000):
    ...
    out["P1_pinn"] = {
        "n_steps": n_pinn,
        "train_s": train_s,
        "rel_l2_mean": rep["mean"],
        "rel_l2_median": float(np.median(rep["rel_l2"])),
        "rel_l2_max": rep["max"],
        "by_kappa": rep["by_kappa"],
    }
    print(
        "  P1 PINN:",
        {k: out["P1_pinn"][k] for k in ("rel_l2_mean", "rel_l2_median", "rel_l2_max")},
        flush=True,
    )
    _save("P_results.json", out)

    # ---- P2: PINO generalising over covariate paths AND params ------------
    op = make_neural_solver(
        backend="jax",
        mode="pino",
        coll_grid=t,
        T=T,
    ...
    P_te = np.column_stack(
        [
            rng.uniform(0.1, 0.7, 64),
            rng.uniform(0.4, 2.5, 64),
            rng.uniform(0.5, 3.0, 64),
            rng.uniform(-0.4, 0.4, 64),
        ]
    )
    ...
    t0 = time.time()
    _ = [
        op.solve(dict(kappa=p[0], theta=p[1], mu=p[2], delta=[p[3]]), t, Z_on_grid=z)
    ...
    out["P2_pino"] = {
        "n_steps": n_pino,
        "train_s": train_s,
        "solve_ms_per_instance": solve_ms,
        "heldout_rel_l2_mean": rep["mean"],
        "heldout_rel_l2_median": float(np.median(rep["rel_l2"])),
        "heldout_rel_l2_max": rep["max"],
        "by_kappa": rep["by_kappa"],
    }
    print(
        "  P2 PINO:",
        {
            k: out["P2_pino"][k]
            for k in ("heldout_rel_l2_mean", "heldout_rel_l2_median", "solve_ms_per_instance")
        },
        flush=True,
    )
    _save("P_results.json", out)
...
def _fixed_operator_dataset(n, M, seq_len, T=30.0, max_radius=0.7, seed=0):
    ...
    S = np.empty((n, seq_len, M), np.float32)
    XI = np.empty((n, seq_len, M), np.float32)
    for i in range(n):
        t, s = _random_forcing(M, seq_len, T, rng)
        forcing = lambda tt, t=t, s=s: s[min(int(np.searchsorted(t, tt, "right") - 1), seq_len - 1)]
        XI[i] = solve_mbpp_ode_multivariate(forcing, A, B, t)
        S[i] = s
    return S, XI
...
def block_F(n_samples=4000, M=6, seq_len=96, epochs=80):
    ...
    def fit_eval(model, Str, XItr, Ste, XIte, ep=epochs):
        Sn, smu, ssd = _standardize(Str)
        XIn, xmu, xsd = _standardize(XItr)
        model.compile(optimizer=tf.keras.optimizers.Adam(1e-3), loss="mse")
        t0 = time.time()
        model.fit(Sn, XIn, epochs=ep, batch_size=128, verbose=0)
        dt = time.time() - t0
    ...
    ntr = int(0.9 * len(S))
    r_fix, dt = fit_eval(
        FourierNeuralOperator(seq_len=seq_len, n_channels=M, width=64, modes=32),
        S[:ntr],
        XI[:ntr],
        S[ntr:],
        XI[ntr:],
    )
    out["F1_fno_fixed_operator"] = {
        "train_s": dt,
        "rel_l2_mean": float(r_fix.mean()),
        "rel_l2_median": float(np.median(r_fix)),
        "rel_l2_p90": float(np.percentile(r_fix, 90)),
    }
    print("  F1 FNO (fixed operator):", out["F1_fno_fixed_operator"], flush=True)
    _save("F_results.json", out)
    ...
    for nl in [0.0, 0.1, 0.25]:
        ...
        if nl == 0.0:
            base = float(r_n.mean())
    out["F4_noise_fixed_operator"] = {
        "clean_test_rel_l2_vs_train_noise": rob,
        "breaking_point": next((k for k, v in rob.items() if base and v > 2 * base), None),
    }
    print("  F4 noise (fixed operator):", out["F4_noise_fixed_operator"], flush=True)
    _save("F_results.json", out)
    ...
    r_ss, _ = fit_eval(
        StateSpaceMBPP(n_channels=M, state_dim=32), Sf[:ntr], XIf[:ntr], Sf[ntr:], XIf[ntr:]
    )
    out["Fdiag_family_unconditioned"] = {
        "diverged_samples_dropped": dropped,
    ...
    print("  F-diag (family, unconditioned):", out["Fdiag_family_unconditioned"], flush=True)
    _save("F_results.json", out)
    ...
    try:
        ...
        ax[0].hist(
            r_fam, bins=30, alpha=0.75, label=f"family, unconditioned (med {np.median(r_fam):.2f})"
        )
        ax[0].set_xlabel("relative L2 error vs exact solver")
        ax[0].set_ylabel("count")
        ax[0].set_title(f"FNO: conditioned vs ill-posed (M={M})")
        ax[0].legend()
        ks = list(rob.keys())
        vs = [rob[k] for k in ks]
        ax[1].plot([float(k) for k in ks], vs, "o-")
        ax[1].axhline(2 * base, ls="--", color="r", label="2x noiseless")
        ax[1].set_xlabel("training-target noise level")
        ax[1].set_ylabel("clean-test rel-L2")
        ax[1].set_title("FNO noise robustness (fixed operator)")
        ax[1].legend()
        fig.tight_layout()
        fig.savefig(os.path.join(RESULTS, "F_operator_accuracy.png"), dpi=140)
        plt.close(fig)
...
def _robust_inference_dataset(n, M, seq_len, T=30.0, max_radius=0.7, theta=1.0, seed=0):
    ...
    C = np.empty((n, seq_len, M), np.float32)
    Gs = np.empty((n, M, M), np.float32)
    i = 0
    while i < n:
        G = _random_branching_matrix(M, 0.4, max_radius, rng)
        A = G * Bm
        mu = rng.uniform(0.2, 1.5, size=M)
        _, Xi = solve_mbpp_ode_multivariate(lambda tt, mu=mu: mu, A, Bm, t, return_compensator=True)
...
def block_A(n_samples=8000, M=5, seq_len=96, epochs=40):
    ...
    t0 = time.time()
    Ghat = net.predict(C[ntr:], verbose=0)
    infer_ms = 1000 * (time.time() - t0) / len(C[ntr:])
    Gte = G[ntr:]
    ...
    out["A1_amortized"] = {
        "train_s": train_s,
        "infer_ms_per_instance": infer_ms,
        "entry_mae": mae,
        "entry_corr": corr,
        "spectral_radius_corr": sr_corr,
    }
    ...
    try:
        ...
        lim = [0, max(Gte.max(), Ghat.max())]
        ax[0].plot(lim, lim, "k--", lw=1)
        ax[0].set_xlabel("true branching entry G[m,j]")
        ax[0].set_ylabel("predicted")
        ax[0].set_title(f"Amortised branching-matrix recovery (M={M}, corr={corr:.2f})")
        ax[1].scatter(sr_true, sr_hat, s=10, alpha=0.5)
        ax[1].plot([0, 1], [0, 1], "k--", lw=1)
        ax[1].set_xlabel("true spectral radius")
        ax[1].set_ylabel("predicted")
        ax[1].set_title(f"Stability recovery (corr={sr_corr:.2f})")
        fig.tight_layout()
        fig.savefig(os.path.join(RESULTS, "A_amortized_recovery.png"), dpi=140)
        plt.close(fig)
...
def main():
    ...
    if args.all or args.block == "P":
        print("=== Block P (JAX physics-informed) ===", flush=True)
        block_P()
    if args.all or args.block == "F":
        print("=== Block F (TF forward operators) ===", flush=True)
        block_F()
    if args.all or args.block == "A":
        print("=== Block A (TF amortised inverse) ===", flush=True)
        block_A()
