...
def run(out_dir="results", seed=0, epochs=7000):
    ...
    t0 = time.time()
    hist = op.train(
        Str, Atr, epochs=epochs, lr=3e-3, batch=96, val=(Sval, Aval), log_every=max(1, epochs // 7)
    )
    ...
    res = dict(
        ...
        rel_l2_test_mean=float(rel_test.mean()),
        rel_l2_test_median=float(np.median(rel_test)),
    ...
    print(
        "  held-out rel-L2: mean %.3f  median %.3f  p90 %.3f"
        % (res["rel_l2_test_mean"], res["rel_l2_test_median"], res["rel_l2_test_p90"])
    )
    print(
        "  train rel-L2 %.3f (vs test %.3f -> generalises)"
        % (res["rel_l2_train_mean"], res["rel_l2_test_mean"])
    )
    print("  per-sector rel-L2: %s" % [round(x, 3) for x in per_sector])
    print(
        "  by branching: low-rho %.3f  high-rho %.3f"
...
def _plot(out_dir, op, hist, t, S, A, xi, XI, rel, rho):
    ...
    a.set_xlabel("epoch")
    a.set_ylabel("residual", color="C0")
    a.set_yscale("log")
    a2 = a.twinx()
    a2.plot(hist["epoch"], hist["val_rel_l2"], "C3-s", ms=3, label="val rel-L2")
    a2.set_ylabel("val rel-L2", color="C3")
    a2.set_yscale("log")
    ...
    a.set_xlabel("relative L2 error")
    a.set_ylabel("count")
    a.legend()
    ...
    a.set_xlabel(r"spectral radius $\rho(A)$")
    a.set_ylabel("rel-L2 error")
    fig.suptitle(
        "A physics-trained neural operator that solves the whole family of "
        "3-sector MBPPs in one forward pass"
    )
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, "exp14_pino.png"), dpi=140)
    plt.close(fig)
