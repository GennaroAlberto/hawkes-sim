...
def run(seed=0, out_dir="results"):
    ...
    cl, el = [], []
    for _ in range(J):
        i2, o2 = simulate_separable_hawkes(
            Constant(5.0, Tj), kappa, theta, Tj, seed=int(rng.integers(1e9))
        )
        cl.append(interval_censor(o2, obsj))
        el.append(MultiImpulse(i2))
    resh = fit_mbpp_bayes_hierarchical(
        obsj, cl, el, endogenous=True, theta=theta, n_samples=2000, burn=800, n_chains=2, seed=0
    )
    ...
    ax.axvline(kappa, color="r", ls="--", lw=1)
    ax.axhline(theta, color="r", ls="--", lw=1)
    ax.plot(kappa, theta, "r*", ms=14, label="truth")
    ax.set_xlabel(r"$\kappa$")
    ax.set_ylabel(r"$\theta$")
    ax.set_title(f"(a) joint posterior: the ridge (corr {corr:+.2f})")
    ax.legend()

    ax = axes[0, 1]
    ax.hist(res.samples["kappa"], bins=40, density=True, alpha=0.7, color="C0", label=r"$\kappa$")
    ax.axvline(kappa, color="r", ls="--")
    ax.set_xlabel("value")
    ax.set_ylabel("posterior density")
    ax.set_title("(b) marginal of $\\kappa$ (tight, covers truth)")
    ax.legend()
    ...
    ax.set_ylabel("posterior density")
    ax.set_title("(c) a $\\theta$-prior legitimately tightens $\\theta$")
    ax.legend()
    ...
    ax.set_title(
        f"(d) hierarchical pooling: $\\kappa_{{pop}}$\n95% CrI [{lo:.3f}, {hi:.3f}] from {J} short series"
    )
    ax.set_xlabel(r"$\kappa_{pop}$")
    ax.set_ylabel("posterior density")
    ax.legend()

    fig.suptitle(
        "Bayesian calibration: honest uncertainty, the identifiability ridge, priors, pooling"
    )
    fig.tight_layout()
    ...
    print(
        f"  theta 95% CrI = {summary['theta_CrI']} (flat) -> {summary['theta_CrI_tight']} (informative)"
    )
