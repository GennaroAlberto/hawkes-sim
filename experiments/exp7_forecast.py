...
from hawkes_calibration.mbpp.exogenous import LHPP, PiecewiseConstant
...
def run(seed=0, out_dir="results", n_videos=12):
    ...
    for v in range(n_videos):
        ...
        views = _simulate_video(
            tweets, kappa_true, theta_true, mu_true, nu_true, seed=int(rng.integers(1e9))
        )
        ...
        rows.append(
            dict(
                ...
                smape_mbpp=_smape(pred, actual),
                smape_base=_smape(base, actual),
                cum_pred=float(pred.sum()),
                cum_base=float(base.sum()),
    ...
    cum_err_mbpp = np.mean(
        [abs(r["cum_pred"] - r["cum_actual"]) / max(r["cum_actual"], 1) for r in rows]
    )
    cum_err_base = np.mean(
        [abs(r["cum_base"] - r["cum_actual"]) / max(r["cum_actual"], 1) for r in rows]
    )
    summary = dict(
        truth=dict(kappa=kappa_true, theta=theta_true, mu=mu_true, nu=nu_true),
        smape_mbpp=smape_mbpp,
        smape_baseline=smape_base,
        cum_err_mbpp=cum_err_mbpp,
        cum_err_baseline=cum_err_base,
        per_video=rows,
    )
    ...
    ax.set_xlabel("video")
    ax.set_ylabel("forecast sMAPE (lower=better)")
    ax.set_title(
        f"Per-video forecast error\n(mean MBPP={smape_mbpp:.3f} vs baseline={smape_base:.3f})"
    )
