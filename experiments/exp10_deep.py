...
from hawkes_calibration import (
    Constant, PiecewiseConstantCovariate, simulate_hawkes_excitation, interval_censor,
    fit_mbpp_ic_excitation, fit_mbpp_ic_sumexp, MBPP, ExponentialKernel, MultiImpulse,
    simulate_separable_hawkes, uniform_obs_times, fit_mbpp_ic, dispersion,
)
from hawkes_calibration.mbpp.interval_censored import _sumexp_compensator_const, _excitation_compensator_fast
...
def run(seed=0, out_dir="results"):
    ...
    Z = PiecewiseConstantCovariate(obs, np.array([[((i // 10) % 2) - 0.5] for i in range(m)], float))
    ...
    for g in range(n_groups):
        counts = []
        for _ in range(K):
            ev = simulate_hawkes_excitation(Constant(mu_t, T), kappa_t, theta_t, Z, [delta_t], T,
                                            seed=int(rng.integers(1e9)))
            counts.append(interval_censor(ev, obs))
        res = fit_mbpp_ic_excitation(obs, counts, Z, n_restarts=3, seed=g)
        deltas.append(res.delta[0]); last = res
    deltas = np.array(deltas)
    summary["excitation"] = dict(delta_true=delta_t, delta_mean=float(deltas.mean()),
                                 delta_std=float(deltas.std()), kappa=last.kappa, theta=last.theta)

    ax = axes[0, 0]
    ax.axhline(delta_t, color="r", ls="--", label=f"true $\\delta$={delta_t}")
    ax.plot(np.arange(n_groups), deltas, "o", color="C0", label="recovered $\\hat\\delta$ (per group)")
    ax.axhline(deltas.mean(), color="C0", alpha=0.5, label=f"mean {deltas.mean():.2f}")
    ax.set_xlabel("group"); ax.set_ylabel("$\\hat\\delta$ (excitation-covariate effect)")
    ...
    Zt = Z(tg)[:, 0]
    ax.plot(tg, kappa_t * np.exp(delta_t * Zt), "k-", lw=2, label=r"true $\kappa(t)=\kappa e^{\delta Z(t)}$")
    ax.plot(tg, last.kappa * np.exp(last.delta[0] * Zt), "b--", lw=2, label="recovered")
    ax.axhline(1.0, color="grey", lw=0.5, ls=":")
    ax.set_xlim(0, 40); ax.set_xlabel("time"); ax.set_ylabel("time-varying branching ratio")
    ax.set_title("(b) recovered excitation modulation")
    ax.legend(fontsize=8)

    # ---- (c) sum-of-exponentials recovery ----
    mu_s = 1.5
    thetas_true = np.array([2.0, 0.4]); kappas_true = np.array([0.15, 0.45])
    ...
    summary["sumexp"] = dict(bank=bank.tolist(), kappas=res_s.kappas.tolist(),
                             total_true=0.60, total_fit=float(res_s.kappas.sum()))
    ax = axes[1, 0]
    ...
    ax.set_xticks(x); ax.set_xticklabels([f"$\\theta$={th}" for th in bank])
    ...
    for _ in range(40):
        imm, off = simulate_separable_hawkes(Constant(5.0, Tg), kappa_g, theta_g, Tg, seed=int(rng.integers(1e9)))
        counts = interval_censor(off, obs_g)
        # correct exogenous (multi-impulse) vs wrong (ignore immigrants -> constant baseline)
        Xi_ok = MBPP(ExponentialKernel(kappa_g, theta_g), MultiImpulse(imm),
                     method="closed").compensator_interval(obs_g, endogenous=True)
        Xi_bad = MBPP(ExponentialKernel(0.1, 5.0), MultiImpulse(imm),
                      method="closed").compensator_interval(obs_g, endogenous=True)
    ...
    summary["gof"] = dict(disp_correct_mean=float(np.mean(disp_correct)),
                          disp_wrong_mean=float(np.mean(disp_wrong)))
    ax = axes[1, 1]
    ax.hist(disp_correct, bins=15, alpha=0.6, color="C0", label=f"correct kernel (mean {np.mean(disp_correct):.1f})")
    ax.hist(disp_wrong, bins=15, alpha=0.6, color="C3", label=f"wrong kernel (mean {np.mean(disp_wrong):.1f})")
    ax.axvline(1.0, color="k", ls="--", label="Poisson (dispersion=1)")
    ax.set_xlabel("Pearson dispersion"); ax.set_ylabel("count")
    ...
    print(f"(c) sum-of-exp total branching: {res_s.kappas.sum():.2f} (true 0.60); weights {np.round(res_s.kappas,3).tolist()}")
