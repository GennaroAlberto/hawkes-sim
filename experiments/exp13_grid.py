...
def make_kernel(kind):
    ...
    if kind == "sumexp3":
        ...
        th = np.array([2.0, 0.7, 0.25])  # 3 timescales, sum w = 1
        return (
            lambda dt: (w[:, None] * th[:, None] * np.exp(-th[:, None] * dt[None, :])).sum(0)
        ), 1.0
    if kind == "powerlaw":
...
def base_excitation_matrix():
    ...
    return np.array([[0.30, 0.08, 0.00], [0.08, 0.30, 0.00], [0.00, 0.00, 0.25]])
...
def run_cell(kernel, cov_level, T=50.0, n_int=25, n_seq=28, n_train=20, seed=0, sector=0):
    ...
    def heldout(fit, Zc):
        Xi = np.diff(
            _excitation_compensator_fast(
                fit.baseline, fit.kappa, fit.theta, Zc, np.atleast_1d(fit.delta), obs
            )
        )
    ...
    out = dict(
        ...
        delta_hat_cov=float(np.atleast_1d(fitc.delta)[0]),
        heldout_ic_ll_nocov=ll0,
        heldout_ic_ll_cov=llc,
        heldout_improvement=ll0 - llc,
...
def clean_sim(T, A, baseline, kernel_fn, Zpop, delta_pop, seed, max_events=20000):
    ...
    bks = np.asarray(Zpop.breakpoints, float)
...
def run_multivariate_cell(T=36.0, n_int=20, n_seq=12, seed=0, full=False):
    ...
    A = np.array(
        [[0.10, 0.26, 0.03], [0.24, 0.10, 0.03], [0.03, 0.03, 0.08]]
    )  # all positive (linear)
    baseline = np.array([1.4, 1.2, 1.5])
    delta_true = 0.5
    counts = []
    for _ in range(n_seq):
        ev = clean_sim(T, A, baseline, kernel_fn, Z, delta_true, int(rng.integers(1e9)))
    ...
    t0 = time.time()
    fit = fit_mbpp_ic_excitation_multi(
        obs, counts, Z, n_restarts=1 if not full else 3, n_sub=3, seed=0
    )
    return dict(
...
def run(out_dir="results", full=False, seed=0):
    ...
    print(
        "=== Experiment 13: factorial grid (%d cells, %s) ==="
    ...
    for kernel, lvl in cells:
        ...
        results.append(r)
        print(
            "  [%-8s | cov=%-5s] delta true=%.2f hat=%+.3f | kappa=%.3f | "
    ...
    mv = run_multivariate_cell(
        full=False
    )  # always the fast correctness size; see docstring to scale up
    print(
        "    %.1fs  delta true=%.2f hat=%.3f  spec.radius=%.3f"
...
def _plot(results, out_dir):
    ...
    dt = [r["delta_true"] for r in results]
    dh = [r["delta_hat_cov"] for r in results]
    ax[0].bar(x - 0.18, dt, width=0.36, label="true", color="0.6")
    ax[0].bar(x + 0.18, dh, width=0.36, label="estimated", color="C2")
    ax[0].set_xticks(x)
    ax[0].set_xticklabels(labels, rotation=40, ha="right", fontsize=7)
    ax[0].set_title(r"recovered covariate effect $\delta_{pop}$")
    ax[0].legend()
    imp = [r["heldout_improvement"] for r in results]
    ax[1].bar(x, imp, color=["C3" if v < 0 else "C0" for v in imp])
    ax[1].axhline(0, color="k", lw=0.6)
    ax[1].set_xticks(x)
    ax[1].set_xticklabels(labels, rotation=40, ha="right", fontsize=7)
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, "exp13_grid.png"), dpi=140)
    plt.close(fig)
