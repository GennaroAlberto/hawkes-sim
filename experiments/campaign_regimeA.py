...
def e1(ds, gt):
    ...
    return dict(
        ...
        dispersion=float(
            np.mean((obs - rates[TRAIN_END:]) ** 2 / np.maximum(rates[TRAIN_END:], 1e-6))
        ),
...
def _heldout(w0, u, eta, ds, fit_mask, cw, eval_mask=None):
    ...
    for t, s, i in ev:
        ...
        if i not in cand:  # funded firm pruned out -> miss
            nll += np.log(cand.size)
            ranks.append(cand.size)
            continue
        lo = max(0, t - cw)
        ...
        nll -= np.log(max(p[pos], 1e-12))
        ranks.append(int((p > p[pos]).sum()) + 1)
    r = np.array(ranks)
    return dict(
        nll=nll / n,
        ...
        n_events=n,
        mean_risk_set=float(eval_mask[TRAIN_END:].sum(1).mean()),
    )
...
def e2(gt):
    ...
    rec = _recency(ds_obs)
    out = {
        "truth_w": [round(float(x), 3) for x in gt["ranker_global"]],
    ...
    for cap in (156, 104, 78):
        m = _fit_eval(ds_obs, ds_obs.active & (rec <= cap), cw)
        m["cap"] = cap
...
def main(out_dir="results"):
    ...
    json.dump(
        res, open(os.path.join(out_dir, "campaign_regimeA.json"), "w"), indent=2, default=float
    )

    print("=== E1: stage-1 sector GLM recovery (regime A, weeks<416) ===")
    print(
        f"  beta corr {r1['beta_corr']:.3f} | excitation corr {r1['excitation_corr']:.3f} | "
    ...
    print(
        f"  held-out Poisson NLL/cell: model {r1['heldout_nll_model']:.3f} vs baseline "
    ...
    for k in ("observed", "oracle", "exit_recency_prune_best"):
        ...
        print(
            f"  {tag:24s} w={m['w0']} eta={m['eta']:.2f} | NLL {m['nll']:.3f} "
    ...
    print(
        "     risk set (keeping first-time funders) is needed. Wrote results/campaign_regimeA.json"
    )
    return res
