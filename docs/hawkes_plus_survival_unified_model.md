...
## 5. Survival stage option A: Cox / McFadden risk-set model
...
The implementation in `sector_survival.py` extends this with an **outside option**.
If a funded startup is not in the tracked universe or watch-list, the event is assigned
to outside rather than forcing the model to pick a tracked firm.
...
4. Use the survival or discrete-hazard stage to compute startup probabilities.
...
The branch now contains an explicit version of this architecture.
...
### Startup survival with outside option

File:

```text
hawkes_calibration/sector_survival.py
```
...
This is the Cox/McFadden partial-likelihood version of the second stage, extended with
an outside option for events that go to firms outside the tracked universe.
...
```text
hawkes_calibration/sector_hazard.py
```
...
This learns from positive and sampled negative active firm-weeks.
...
sector count model
risk-set ranker
startup survival model with outside option
...
1. **Sector Hawkes/count model + startup survival stage.** This is the cleanest and
   most interpretable version.
...
- Cox/startup survival;
- Hawkes + startup survival;
