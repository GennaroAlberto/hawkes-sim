...
Measured verdicts from the rehearsal (\code{exp28}): the nested design (N3)
attains the best joint likelihood on both worlds, the context-ASC (N1) the best
quarterly newcomer-share calibration (mean $\approx$3--4pp error vs.\
$\approx$6--9pp for a bare constant); the integrated entrant-quality term (N2b)
explains only 13--15\% of the fitted ASC, so most of the newcomer utility is
coverage and pool-size, not entrant quality; and the immigrant stream carries
the larger fitted ``excitation'' radius ($\approx$0.30 vs.\ $\approx$0.08--0.11
for repeats) --- entry waves absorbing the latent factor, exactly the S1
mechanism. The practical recommendation is N3 for likelihood, N1 for
calibrated shares, and the two coincide in their incumbent weights.
...
Measured: newcomer wins split $\approx$45/45/10\% between true entrants,
untracked incumbents, and pool misses, yet separating the two classes improves
joint likelihood by only $+0.003$ nats --- the conflation is nearly free in
likelihood even though it blocks compositional interpretation.
...
(\S\ref{sec:design_bench}). \emph{Status: this test has already fired on the
synthetic worlds} --- parity XGBoost beats the logit by $+0.22$ nats and
$+9$pp incumbent top-5 on world A ($+0.10$ nats on B), so the linear utility
leaves genuine nonlinearity (plausibly gap-feature interactions) on the table
even before feature enrichment. The expectation for real data is therefore
revised: budget for a nonlinear mark scorer or engineered interactions, and
treat the parity gap, re-measured on real data, as the enrichment budget.
\end{enumerate}
...
features/nonlinearity, and the gap size is the budget for enrichment. In the
synthetic rehearsal the gap materialized (parity $+0.22$ nats on A), so the
linear-utility restriction is already known to be falsifiable in practice and
the mark factor's robustness claim is scoped to the \emph{likelihood family},
not the linear score.
