...
the market's own history is not. Their role in the design is deliberately
adversarial. Every richer timing model in this document --- the
...
\cref{ch:gof}. The rehearsal campaigns justify this posture with numbers:
in the decomposition benchmark, covariates carried almost the entire
held-out gain over a seasonal baseline ($+3.5$, $+5.7$, and $+1.5$ nats
per sector-week across the three synthetic worlds), while everything added
on top of them --- lags, excitation, cross-sector terms --- contributed
decimal points (\cref{app:results}). A Hawkes fit whose covariate baseline
...
macro cycle, which is exactly what this chapter calibrates.

The second reason to linger at this rung is that it is the one rung whose
...
(\cref{ch04:sec:censoring} proves this and thereby settles a standing open
question of the repository), interval censoring by weekly aggregation costs
nothing (\cref{thm:ch04:glmequiv}), and sectors neither enter late nor die,
so (C2) and (C3) do not bind. The Poisson rung is where calibration
discipline is cheap; every deviation from it that we later purchase must be
priced against a baseline whose own accounting is beyond suspicion.
...
message is the level. It is the null of the whole timing enterprise; its
calendar-dummy refinement (a rate per quarter or per season) is the
``seasonal baseline'' against which the covariate gains above are measured.
...
Three properties recommend $\exp$, and each earns its keep somewhere
specific in this document. \emph{Positivity for free}: any $\bcoef \in
...
taming the exponential extrapolation risk that haunts the log link outside
...
rehearsal twin \code{fit\_sector\_glm\_fast(n\_lags=0)} --- computes
...
This section closes a standing open question of the repository: what must
Poisson calibration do about right censoring? The answer, proved here in
...
Funding counts violate this: latent heterogeneity (unmodeled sector-week
quality, vendor coverage drift) and genuine history dependence both
inflate variance beyond the mean, and the rehearsal measured quasi-Poisson
dispersion $2.72$ on the misspecified regime-B weekly sector fit
(\cref{app:results}) --- weekly variance nearly three times the Poisson
prediction. Two upgrades
...
consistency under overdispersion; only its advertised precision was
wrong. Quasi-Poisson repairs the advertisement at zero modeling cost, but
...
heterogeneity the covariates miss. That reading sets the boundaries of
the upgrade honestly. Independent frailties per cell fix the
...
statistic can demand escalation but can never certify excitation, and
the rehearsal's $2.4\times$ branching-ratio inflation under latent
AR frailty (\cref{app:results}) is the standing reminder of what happens
when clustered variance is read as contagion. The frailty-vs-contagion
...
outputs. Rationale: measured overdispersion (dispersion $2.72$ on the
stress world, \cref{app:results}); an exact frailty interpretation
...
One step remains inside GLM-land, and the repository's sector layer
already stands on it. Add the recent past to the design: lagged own- and
...
ladder: \emph{Hawkes without Hawkes}. Fixed lags (or fixed-decay filters)
play the role of a fixed kernel, capturing the predictive content of
excitation while refusing to estimate the timescale --- the one parameter
that weekly data cannot identify anyway (the $\branch$--$\decay$ ridge;
\cref{ch:gof}).
...
not a probability of anything \citep{kalbfleisch2002}. Second, and this
the design must state loudly because the repository's own history
includes the failure mode:
...
that the theory requires, and the rehearsal produced a measured
metastable excursion in a configuration whose nominal spectral radius was
comfortably subcritical (\cref{app:results}). Design rules that follow:
...
is the vectorized rehearsal twin used by the exp27-style decomposition
benchmarks. \modrepo{eventtime/estimate.py}
