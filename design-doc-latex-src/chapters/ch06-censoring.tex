...
decides what to do when it is not. The thesis is stated once and defended
throughout: \emph{the correct response to untrusted positions is to choose
...
exactly that and nothing more. One reassurance before the hard work: the
...
\section{Interval censoring for Hawkes: where exactness dies}\label{ch06:sec:ichawkes}
...
for a likelihood that must be refit across sectors, splits, and rehearsal
...
overdispersion --- is stated, bounded, and paid with open eyes.
...
Because precision about approximations is this document's currency, we
state exactly what \cref{eq:ch06:mbppll} gets right and wrong when the data
is generated by the Hawkes process itself.
...
$(1-\branch)^{-2}$ larger per bin --- at the rehearsal's
$\branch\approx0.35$ a ceiling of $\approx 2.4$, i.e.\ na\"ive standard
errors up to $\approx (1-\branch)^{-1}\approx1.5\times$ too small --- and
positive cross-bin correlation compounds the understatement. (This
$2.4$ is a derived variance ceiling, unrelated to the $2.4\times$
branching-ratio inflation under latent frailty recorded in
\cref{app:results}.) Remedies, in committed order: (i) sandwich
...
ceiling $(1-\branch)^{-2}$, which is $O(1)$ and known; the fitted
$\branch$ on this data lives near $0.3$--$0.4$, far from the boundary.
...
only through the relaxation root $e^{-r\horizon}$. The rehearsal campaigns
made this quantitative (inherited results, \cref{app:results}): the
Fisher information for $\decay$ carried by counts decays like
$\horizon^{2}e^{-2(1-\branch)\decay\horizon}$ --- exponentially in the bin
width --- so binning does not merely blur the timescale, it erases it. At
daily bins the misspecified regime-B world ($\decay = \ln 2/7$ per day)
was recovered unbiased; at weekly bins ($\approx$ one half-life)
essentially nothing was lost; at 28-day bins ($\approx 4$ half-lives) the
$\decay$ estimate was biased low by $\sim20\%$ \emph{with a deceptively
small spread} --- the signature of an unidentified direction collapsing
onto a ridge --- while $\branch$, read from first moments, stayed within
$0.28$--$0.38$ of its true $0.35$ at every width. Hence the standing rule,
inherited as a veto: \emph{bucket width $\le$ one kernel half-life is safe
for $\decay$; $\ge$ four half-lives destroys timing; $\branch$ and
covariate effects survive any width}. The Bayesian implementation
...
reported next to any timing claim. The corresponding failure mode is
seductive precisely because the fit \emph{converges}: the optimizer happily
returns a $\decay$ with a small na\"ive SE while sliding along the ridge.
\end{warning}
...
coarse bins must \emph{reproduce} the known low-bias signature, not
surprise us; (2) SE honesty --- the ratio of Monte Carlo spread to
...
--- the transfer convention of \cref{ch02:sec:synthetic}.
\end{enumerate}
\end{protocol}
...
\cref{ch06:sec:ridge} (weekly $\approx$ one half-life of the regime-B
kernel: safe for $\branch$, marginal for $\decay$). Reversal trigger: a
