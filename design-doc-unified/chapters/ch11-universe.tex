...
Every model developed so far has assumed that we know who could
receive the next deal. This chapter treats the two ways that assumption
fails. First, deals arrive from firms that are not in the tracked
...
we model --- defect (C2). Neither failure is negligible: the synthetic
world is built with a material newcomer share, and
\cref{ch:results-synth} will measure how much of the deal stream falls
outside the current risk set and how that share behaves in held-out
windows. A mark factor that ignores this mass overstates every tracked
firm's probability by the factor $(1-\pi)^{-1}$ of
\cref{eq:ch11:inflation}; a ground model that ignores it cannot
forecast sector totals honestly. The remedy of this chapter is
structural: an explicit outside option in the mark factor, an external
component in the ground process, a model-free audit of the unseen mass,
and an entry model for the population side of (C2).
...
format, and it is the central asymmetry of this chapter: sector-level
ground counts are \emph{complete} --- every deal has a sector --- while
firm-level attribution is complete only on $\firms$. We state the
consequence as a result, because it decides which layers of the
architecture (C6) can hurt.
...
For illustration, a newcomer share of $\pi = 0.25$ makes the inflation
in \cref{eq:ch11:inflation} a $33\%$ overstatement on every tracked
...
Before building the repair, one distinction must be fixed, because
every downstream interpretation depends on it.
...
coverage catching up). The two channels are an unidentified mixture. We
expect the conflation to cost little in fit and to be undetectable by
fit --- the synthetic world, which carries oracle channel labels, can
test this (\cref{ch:results-synth}). What it costs, permanently,
...
The warning has a converse worth stating: if the two channels are
indistinguishable in likelihood, a single outside option that absorbs
both should lose little as a \emph{predictive} device. The
...
Two estimation remarks. First, if the newcomer share is material the
...
context features in \cref{eq:ch11:outsidescore} exist to make the
newcomer share \emph{calibratable over time}; whether they do, and how
much they improve on a constant share, is a planned synthetic test
(\cref{ch:results-synth}), with the acceptance check fixed by
\cref{prot:ch11:reliability}. The trailing newcomer share
...
The equivalence matters for one practical reason: it makes the
\emph{group-level variant} a first-class alternative rather than an
architectural change. Sector labels carry mislabel noise
...
share, reporting the mean absolute error next to a constant-share
baseline; (b) pool deals into bins by predicted $\pi$ and
...
estimation), and the expected calibration gain from context covariates,
to be verified in \cref{ch:results-synth}.
...
outside the tracked system's genealogy. One caution tempers the
interpretation: entry waves are where latent common factors live, so
apparent ``excitation'' fitted on the newcomer substream must be read
as clustering of unknown origin, in line with the frailty-inflation
argument of \cref{ch:hawkes} (fitted branching ratios are upper
bounds). Fitting the two substreams separately and comparing their
apparent excitation is a planned check for \cref{ch:results-synth}.

\subsection{The histories rule, and the price of breaking it}\label{ch11:sec:histories}

The excitation term in \cref{eq:ch11:mudecomp} raises the sharpest
operational question in this chapter: \emph{which deals enter
...
same understatement, arrived at differently. If covariates dominate the
likelihood, as we expect (\cref{ch:results-synth} will test this on
synthetic data), excitation has little likelihood mass to begin with;
\cref{prop:ch11:thinning} says that breaking the histories rule removes
a $(1-p)$-sized share of exactly that mass.
...
disagree informatively. The classical tool fits our question exactly:
\emph{what is the probability that the next deal comes from a firm we
...
audit of \cref{ch11:sec:drift} clears it). One working hypothesis, to
be tested in \cref{ch:results-synth}: newcomer utility is dominated by
the alternative-specific constant and the context/pool-size terms
rather than by entrant-profile quality. If that holds, effort belongs
in $g_{s,t}$ and the cohort structure, not in rich profile models.
...
\code{build\_choice\_sets(newcomer\_context=True)}; prior art in
\code{experiments/exp28\_newcomer\_designs.py}
