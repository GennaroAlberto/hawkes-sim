...
risk in sector $s$ at $t^-$. This chapter sharpens that framing: when
T1 is the valuable thing, the likelihood of the full marked process is
not the metric --- the ranking is. The full likelihood rewards timing
accuracy that no action consumes and that the data defects of
\cref{ch:data} corrupt. The object to be scored is the ordered list,
and the object to be fitted is the conditional law
\cref{eq:ch10:target}, whose own likelihood --- the conditional NLL of
the mark factor in \cref{thm:ch03:groundmark} --- is the proper
training loss because it is a likelihood of the ranking target and of
nothing else.
...
because every robustness claim in this chapter follows from the form of
the identity: a ratio.
...
The ratio form is the key observation. Any factor common to the
...
untrusted-date defect, and is insulated from ground-process
...
assumption rather than a hope. Whether the device works under
misspecification is a planned synthetic test: in regime B of the
synthetic world the generator's inhibition is a continuous
30-day-half-life kernel, not a binary window, and
\cref{ch:results-synth} will measure how well the fitted mark-factor
cooldown recovers the generator's self-inhibition \emph{ordering}
across firms. The binary window $C_i(t)$ with default
$K_c = 26$ weeks is a one-column basis for a decaying curve;
...
The linear score \cref{eq:ch10:score} is a restriction, and we do not
expect it to hold. The B1 benchmark of \cref{ch:results-synth} tests it
directly: a feature-parity XGBoost ranker against the linear
conditional logit, scored on held-out conditional NLL and top-5 hit
rate. If the restriction fails, the design response is not to abandon
the conditional-choice likelihood but to change the score family inside
it: replace $\score_i(t)$ by $f(\zfeat_i(t), C_i(t), \staleage_i(t),
\dots)$ with $f$ an additive tree ensemble
\citep{friedman2001,chenguestrin2016}, keeping the softmax over the
current risk set and the loss \cref{eq:ch10:nll} exactly as they are.
Training requires only the per-candidate gradient and Hessian of the
grouped-softmax loss; the ``listwise'' machinery of the learning-to-rank
literature reduces to a short calculation.
...
\cref{thm:ch10:sampled} keep the objective correct at scale. The same
boosted learners can also parameterize a continuous-time intensity
directly through a binned likelihood; \cref{ch:nonparam} develops that
intensity-side counterpart.
...
them. None of them certify the \emph{linear} score, which the B1
benchmark of \cref{ch:results-synth} will test, and none of them certify
a boosted scorer trained on a different loss (a pairwise surrogate, a
pointwise classifier): change the likelihood family and the cancellation
proofs no longer apply. When this chapter says ``the ranker is robust to
(C1) and (C4),'' it means the grouped-softmax likelihood is; claims
about any particular fitted score function are empirical and belong to
\cref{ch:results-synth} and \cref{ch:results-spain}.
\end{warning}
...
same risk set. Survival analysis has the same situation:
...
\sim 10^{-1}$ costs tenths of a nat per cell). Since same-week
multiplicity is common in hot sectors --- precisely the cells the
business cares about --- the exact likelihood is worth its
implementation cost. One complication:
...
Rationale: \cref{eq:ch10:plbias} gives a systematic bias against
concentrated score profiles that can reach tenths of a nat per hot
cell; the Cox-ties literature says the exact method dominates Breslow
whenever ties are heavy \citep{therneau2000}. Reversal trigger: if on real data the
...
aimed at NDCG \citep{burges2010,jarvelin2002}. The question deserves a
...
score-family misspecification --- whose extent \cref{ch:results-synth}
will measure.
...
the truth. That trade can be real, but it has a cost we refuse to pay in
...
synthetic data, the oracle ranker (ceiling); synthetic reference values
will be reported in \cref{ch:results-synth}, in the format of
\cref{tab:ch10:rehearsal}.
...
series rather than overwriting one. Rationale: we expect risk-set
truthfulness to be the largest sensitivity in the system ---
\cref{ch:results-synth} will quantify the gap between observed and
oracle membership --- so an unversioned membership rule would make
every metric movement uninterpretable.
\end{enumerate}
\end{protocol}
...
\bottomrule
\end{tabular}
\caption{Reference format for the mark-factor ranker baselines on
synthetic data; the fitted, oracle, and random columns will be filled
by the synthetic campaign of \cref{ch:results-synth}. The oracle
ceiling is expected to sit well below $1$: the residual is irreducible
softmax stochasticity, not model error. Synthetic values calibrate
expectations for real-data campaigns; they are priors and vetoes,
never substitutes (\cref{ch02:sec:synthetic}).}
...
boosted scores are chosen over linear within the family because we
expect the linear restriction to fail, with the B1 benchmark of
\cref{ch:results-synth} as the planned test; and
...
becomes the conditional law given the contaminated pool. We expect this
to be the largest sensitivity in the whole system;
\cref{ch:results-synth} will quantify the gap between observed and
oracle risk sets and test whether a discriminative exit classifier can
close it. Mitigations are the exposure rules of \cref{ch:buckets}
...
caution follows from the theory: the optimal score weighting
\emph{given a contaminated pool} differs from the oracle weighting ---
the fitted model partially adapts to contamination --- so cleaning the
risk set changes the optimal parameters, and oracle-world or
cleaned-world coefficients must never be transplanted onto contaminated
risk sets or vice versa. Risk-set definition and fitted model are a
matched pair; \cref{prot:ch10:recall}(g) versions them together.
...
\modrepo{models/hawkes\_ranker.py} carries the regime-B synthetic ranker
used for the planned cooldown test of \cref{ch10:sec:cooldown}.
...
random/oracle baseline columns in the format of
\cref{tab:ch10:rehearsal}. (iv) The
