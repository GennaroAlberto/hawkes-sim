...
poses: will firm $i$ have a deal within the next $\horizon$? The object
is the fixed-horizon event probability $p_i(t,\horizon)$ of
\cref{eq:ch01:t3}, consumed by binary, capacity-constrained actions ---
enter diligence now, keep on the watch list, deprioritize. The natural
approach is supervised learning on a bucketed panel: turn the event
history into rows ``firm $i$ at time $t$, did it raise within
$\horizon$,'' and hand the rows to a learner that handles nonlinearity.
This chapter commits to that approach, with gradient boosting as the
learner, and treats it as statistics. The bucketed panel is not a
machine-learning convenience outside the point-process theory of Part
II; it \emph{is} a discrete-time hazard model, with an exact link to the
conditional intensity (\cref{thm:ch09:hazardlink}), a censoring theory
(\cref{ch09:sec:censoring}), a sampling theory
(\cref{ch09:sec:sampling}), and a dependence structure that rules out
the usual i.i.d.\ habits (\cref{ch09:sec:dependence}). Every labeling
shortcut --- what to do with windows that cross the end of the data,
whether a firm that died quietly is a negative, whether sliding the
...
One remark on the label \cref{eq:ch09:label}: it depends
...
them injects a bias with a known sign, and more data will not fix it. Reversal trigger:
...
The lemma has a practical reading: the bucket probability
...
along the no-event paths. The panel model therefore estimates the
compensator increment of Part II, coarsened.
...
Part (iii) has a practical reading. When per-bucket probabilities
...
is \emph{not} absorbed by an intercept: logistic coefficients stop being
transferable across horizons, while cloglog coefficients remain the same
$\bcoef$ at every $\horizon$ by \cref{eq:ch09:cloglog}. In this precise
sense cloglog, not logit, is the aggregation-coherent link. It is also
why the calibration layer of \cref{ch09:sec:calibration} is mandatory:
it repairs, on the validation era, exactly the link distortion bounded
here.
...
a design choice. There, weekly binning was an observation defect (C4)
that destroys decay information while sparing the branching scale; how
fast recovery degrades as the bin width grows past the kernel half-life
is a question for the planned synthetic study (\cref{ch:results-synth}).
Here the same mathematics appears with the roles reversed: T3 does not
\emph{want} within-horizon timing, so we choose $\horizon$ to integrate
it out, and \cref{cor:ch09:defect} states exactly what is discarded.
The defect is not an accident; it is accepted at a known price because
the consuming action is horizon-bound.
...
so the bias looks like an era effect and contaminates the macro
...
naive risk set and contributes an endless string of structural
negatives. The expected consequences follow from \cref{ch:data}:
hazards deflate by the contamination fraction, and covariates
correlated with staleness can acquire \emph{inverted} weight, because
the optimal response to a pool containing dead firms is to bet against
staleness. The synthetic study of \cref{ch:results-synth} is designed
to measure both effects. Three mitigations are committed, in order of
force. First, \emph{exposure windows}: a row $(i,t_b)$ requires
observed activity (a deal, or a point-in-time exit proxy refresh)
within the trailing $E$ years. Because the rule is a predictable
function of the observed history, it is exactly independent censoring;
its benefit is not repairing the mechanism but capping each dead firm's
phantom exposure at $E$ years. Second, an explicit \emph{active model}
--- a point-in-time classifier of ``still alive'' --- used as a
feature, not as a hard filter: we do not expect even a good
discriminative exit model to reconstruct truthful risk sets, a
hypothesis \cref{ch:results-synth} will test, so it may inform but must
not silently redefine the estimand. Third, the staleness age itself
partially absorbs the defect: dead firms accumulate very large
$\staleage$, and a flexible learner routes their predicted probability
toward zero through the $\staleage$ feature --- which is also why the
fitted $\staleage$ effect is \emph{not} interpretable as the true
renewal profile (it mixes the genuine gap dependence with the
contamination). The residual bias after all three mitigations is
signed: fitted hazards remain biased \emph{downward}, most severely at
large staleness ages, and every probability this chapter emits for a
long-quiet firm should be read as a lower bound in that direction.
...
The panel looks like a supervised-learning table but is not an i.i.d.\
sample. Rows share firms across buckets: the latent quality that
\cref{ch:stale} models as a persistent state makes $y_{i,b}$ and
$y_{i,b'}$ positively dependent for all bucket pairs of the same firm.
Rows share time within buckets: macro regimes and sector cycles are
common shocks, and we expect this shared structure to carry much of the
signal --- a hypothesis the synthetic study of \cref{ch:results-synth}
will test. Both dependencies survive conditioning on the features
whenever the features are incomplete, which defect (C5) guarantees.
The effective sample size is therefore far smaller than the row count,
nominal i.i.d.\ standard errors are wrong, and shuffled
cross-validation leaks by construction (rows from the same firm and era
land on both sides of the fold).
...
exact likelihood and honest uncertainty; we expect between-era variance
to dominate, so the discarded overlap should buy little
(\cref{ch:results-synth} will check). Reversal trigger: if the $\horizon=104$
...
panel --- and both have exact treatments; neither justifies ad hoc
reweighting.
...
training of \cref{ch:ranking} and \cref{eq:ch09:subsample} are the same
result in two forms: a known sampling scheme induces a computable
likelihood correction, so sampling is unbiased whenever the scheme is
recorded.

For the boosted trees themselves the common alternative is
...
explicit regularization. The same learners appear again in
\cref{ch:nonparam} with a different target: here the tree model fits a
discrete-time hazard on the bucketed panel, while in \cref{ch:nonparam}
the same learners parameterize a continuous-time intensity through the
binned likelihood --- same tooling, different target. We derive the
fitting rule once, because two of its internals (the Hessian
denominators and the default directions) are load-bearing for our data.
...
\paragraph{Why boosted trees.} Four reasons. First, \emph{the staleness
interaction is the model}: \cref{ch:stale} establishes that the
predictive content of a LOCF covariate decays with its age, so the
truth has the form $f(\tilde\xcov, \staleage)$ with effect modification
in $\staleage$ --- exactly what axis-aligned splits represent cheaply
and what a linear cloglog must hand-engineer term by term. Second,
PitchBook columns are mixed-type: round-type categories, flags, sizes
spanning orders of magnitude, and pervasive missingness; XGBoost's
sparsity-aware default directions \citep{chenguestrin2016} learn, per
split, which way missing values go --- and since missingness here is
informative (undisclosed size is a signal, not an accident), learning
the direction is modeling, not imputation. Third, nonlinearity in the
engineered history (trailing counts, raised-to-date) is expected and
cheap to capture. Fourth, we expect the linear-utility restriction to
fail. The planned B1 benchmark of \cref{ch:results-synth} tests XGBoost
against the linear conditional logit at strict feature parity --- the
same features, the same choice sets, the same temporal split --- and
the panel's feature set is wider and dirtier than any setting in which
linearity is plausible. The linear cloglog GLM remains in the harness
as the benchmark every boosted fit must beat, but it is the challenger,
not the champion.

\paragraph{Hyperparameters that matter.} Depth (\code{max\_depth})
bounds interaction order; we expect gains at modest depth (3--5), with
deeper trees mostly overfitting firm identity.
...
We decline to impose the tempting constraint that intensity decreases
in the staleness age $\staleage$. The true gap dependence
...
pipeline pacing. A model that ranks well but emits probabilities off by
a factor of two corrupts every one of those computations, so
calibration is mandatory infrastructure. Three distortions are
anticipated and named: the
...
The repository has prior art: experiment \code{exp16} implements a
discrete-time logistic-hazard model with a Kaplan--Meier calibration
reference and a pairwise ranking-loss hybrid on synthetic firms. The
panel stack of this chapter builds that sketch out fully, with the
labeling, censoring, and sampling theory made explicit.
...
without calibration is not enough here. All uncertainty is
...
must be stated explicitly. The two targets differ in
...
identical origins is the cross-stack audit demanded by design decision
D1.2: when the two stacks disagree systematically, one of them is wrong
about something specific, and the diagnostics in this chapter exist to
locate it.
...
same harness so that every gain is measured at feature parity.
\end{codehooks}
