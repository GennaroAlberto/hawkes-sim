...
improves. The earlier chapters built the outputs: a sector-level
ground intensity (\cref{ch:poisson,ch:hawkes,ch:censoring}), a
firm-level conditional ranker with an outside option
(\cref{ch:ranking,ch:universe}), and horizon event probabilities
(\cref{ch:buckets}), all under the evaluation rules of
...
deliverables, the decision rules that consume them, and the
feedback loop that measures whether the system earns its keep. The
layer is deliberately thin. It contains no new statistical
estimation --- only composition, thresholds, and reporting. Every
...
For each sector $s$ at time $\tau$, it lists the top-$K$ tracked firms by
conditional next-deal probability $\hat p(i \mid s, \tau)$
(\cref{ch:ranking}). Each firm is annotated with its horizon probabilities
...
within $\horizon$). A firm can top the ranker while all its horizon
probabilities are low, because the sector itself is quiet. That pairing
tells the desk to watch rather than act.
...
with predictive intervals. We produce them by simulating the fitted ground
...
Every pacing chart carries two fixed footnotes: the
...
step conditional on a deal --- put a candidate in context. They use the
same gradient-boosting stack as \cref{ch:buckets}, applied to deal-level
targets from \cref{app:columns}, with staleness features and with the
...
Offline metrics are proxies. The loop closes only when we measure
...
rolling-origin protocol of \cref{ch:gof}, with three planned tripwires,
each mapped to its owning chapter: (i) calibration drift on horizon
...
Retraining without a tripped wire is discouraged: it churns the
