...
\section{The evaluation protocol}\label{ch07:sec:constitution}

Every preceding chapter builds an estimator; this chapter fixes the rules
by which all of them are judged. Its deliverables are not models but rules:
a definition of ``calibrated'' for each of the four targets of
...
the caveats it needs. We call this the \emph{evaluation protocol} and mean
it to be hard to amend: metrics chosen after seeing results are not
metrics. We expect the difference between a defensible claim and an
embarrassing one to lie rarely in the model and mostly in whether the
evaluation respected the risk set, the temporal split, and the
identifiability limits of the parameter being quoted; the synthetic
campaigns of \cref{ch:results-synth} are designed to test exactly that.
...
\begin{decision}{D7.1 --- the evaluation protocol}
...
protocol only together with a new consuming action, per decision D1.1
...
cheapest way to fool ourselves; with enough scores to choose from, every
family looks good on \emph{some} score. Reversal trigger: a change
...
not been run against all three has not been evaluated. The challenger
families include the parametric intensity models of
\cref{ch:poisson,ch:hawkes}, the count models of \cref{ch:buckets}, and
the nonparametric intensity models of \cref{ch:nonparam} --- the last
admitted only on trusted-date subsets, per \cref{ch:censoring}.
...
\begin{decision}{D7.2 --- NLL ladder with the covariates-dominate hypothesis}
...
The working hypothesis is that \emph{covariates dominate and excitation is
decimal points}; the synthetic campaigns of \cref{ch:results-synth} will
test it, and \cref{ch:results-spain} will report the real-data ladder.
Until it is overturned, any claim that excitation matters must clear the
seasonal NHPP \emph{and} the covariate NHPP under rolling-origin
evaluation, not merely improve on a weak baseline. Reversal trigger:
excitation contributing $\ge 0.5$ nats per cell over the covariate NHPP on
real data in two consecutive campaigns retires the hypothesis.
\end{decision}
...
The predicted consequence, to be checked in the recovery experiments of
\cref{ch:results-synth}: at coarse bucket widths, $\decay$ should collapse
onto the ridge --- biased, with a deceptively tight spread, the signature
of an unidentified direction --- while $\branch$ stays stable at every
width. The working rule, restated from \cref{ch:censoring}: bucket width
at most one kernel half-life is expected to be safe for timing; four or
more half-lives is expected to destroy it; in the destroyed regime, fix
$\decay$ to a decay bank and report
...
channels both into $\hat\branch$. A planned synthetic campaign
(\cref{ch:results-synth}) will quantify the masquerade: fit the Hawkes
model to worlds driven by latent frailty with no contagion and measure
how much spurious excitation it reports.
...
interior points --- the repository's standing discipline; a family whose
gradients do not check does not proceed to fitting.
...
\item no ranking metric without its \emph{risk-set definition}: the risk
set, not the model, can be the largest term in a ranking metric, and the
synthetic experiments of \cref{ch:results-synth} will quantify the
observed-versus-oracle gap;
\item no count NLL without its \emph{dispersion} printed alongside
(\cref{prot:ch07:pit}).
\end{enumerate}
Rationale: each naked number can mislead, and its caveat can reverse the
...
(\cref{ch:poisson,ch:ranking}), why an exit classifier alone --- however
discriminative --- cannot certify the repair, and why rule (i) of
...
\modrepo{eval/}, the evaluation harness that makes the protocol
