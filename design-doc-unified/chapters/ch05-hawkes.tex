...
market. If their residual diagnostics show clustering beyond what
covariates explain --- a hypothesis the studies of \cref{ch:results-synth}
will probe under controlled conditions --- the Hawkes process
\citep{hawkes1971} is the
...
(\cref{ch05:sec:simulation}), and then the central section:
what to do about \emph{inhibition}, which a nonnegative-kernel Hawkes
process provably cannot express, and which we expect in this market --- a
firm that has just raised is temporarily \emph{less} likely to raise again
...
$\branch$--$\decay$ ridge analyzed in \cref{ch:gof}. Our working
hypothesis, to be tested in \cref{ch:results-synth}, is that $\branch$
survives aggregation while $\decay$ does not. A parameterization in which the kernel height
...
theorem: the process is a Poisson cluster process
...
$33\%$ of deal flow attributed to contagion --- a large error.
...
Three practical points. \emph{Boundary corrections.}
...
We expect the market to have a signed dynamic: sector-level deal flow
\emph{self-excites} (capital attention, momentum), while a firm's own
recent raise \emph{inhibits} (a funded firm leaves the market for
months). Regime~B of the synthetic world builds in exactly this
...
\subsection*{Route (a): log-link self-modulated intensity}
...
The log-link model contains the linear Hawkes model to first order.
...
\cref{eq:ch05:nesting}. If, as we expect, covariates dominate and the
excitation load is small, the surrogate error is second order in an
already small quantity; \cref{ch:results-synth} will measure the load.
The overshoot has a
sign, however: the log-link \emph{amplifies} large excursions that the
linear model would meet additively, and that is the source of the
instability below.
\end{proof}
...
\emph{absorbing rare event}, invisible to in-sample diagnostics. We
expect simulation to show exactly this failure mode: long metastable
excursions in configurations whose linear-style spectral radius is
nominally far subcritical. The stress tests of \cref{ch:results-synth}
are designed to exhibit it. The theory
...
route (b) would cut off the machinery that \cref{ch:censoring} needs.
It is held in reserve as the escalation for a genuinely signed
continuous-time ground kernel.
...
(\modrepo{sector\_stability.py} + \modrepo{sector\_ranker.py}). We expect
the self-inhibition \emph{ordering} recovered through the ranker cooldown
to stay robust even when the parametric shape of the cooldown is wrong;
the misspecification experiments of \cref{ch:results-synth} test this.
...
factor. The frailty world of \cref{ch:results-synth} is designed to
measure the size of this inflation under a persistent latent factor.
...
common factors inflate this estimate,'' citing the frailty calibration of
\cref{ch:results-synth} as the measured severity once that study is run;
(iii)
...
the documented sign (downward on firm-level intensities). We expect the
risk-set sensitivity to be among the largest in the system;
\cref{ch:results-synth} will measure it, and treatments live in
\cref{ch:ranking}.
