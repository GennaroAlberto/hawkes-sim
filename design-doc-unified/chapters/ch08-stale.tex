...
current truth grows with the time since the firm's last deal. Second, the
observation times
...
The stakes justify the chapter's rank. Design decision D2.1 of
\cref{ch:data} predicts that the largest modeling errors will come not
from wrong model families but from unstated exposure to (C3) and (C5).
The covariates-dominate hypothesis of \cref{ch:gof} places most of the
predictive mass in the covariate channel, and a planned covariate-noise
campaign (\cref{ch:results-synth}) will measure how staleness erodes
fitted structure before it erodes prediction. A defect that silently
corrupts the dominant channel, differentially across the risk set,
...
face of the same mathematics, the planned synthetic validation, and the
defect declaration (\cref{ch08:sec:buckets,ch08:sec:declaration}).
...
How much information does the age itself carry? We treat this as an
empirical question for \cref{ch:results-synth}. The sign is not obvious:
in a cooldown world, recent activity can be anti-signal at short range,
so a recency-only ranker need not beat even a popularity baseline. The
committed choice models carry an explicit gap covariate because the gap
is informative, but its fitted functional form is expected to depend on
the generating process, so it must be validated rather than assumed. The
age is neither ignorable nor
...
\citep{prentice1982,carroll2006}. The planned covariate-noise campaign
realizes exactly this benchmark (\cref{ch08:sec:buckets}).
\end{remark}
...
andersengill1982}. This is not a heuristic patch:
...
$\bcoef$ and the drift parameters --- the frailty-versus-contagion
confounding of \cref{ch07:sec:identifiability} returns
...
\section{The bucket-model face, and the planned measurement}
...
learn the interaction surface. If the boosted-ranker benchmark of the
evaluation plan finds a gap between trees and the linear conditional
logit, part of that gap may be exactly this staleness interaction; how
much is an isolation question assigned to \cref{ch:results-synth}.

The attenuation mechanics themselves will be measured by a planned
covariate-noise campaign, reported in \cref{ch:results-synth}. It has
two stages. Stage one applies exogenous, uninformative Gaussian noise to
the covariates at fit and prediction time --- the \emph{classical-error
benchmark} of \cref{ch08:sec:attenuation} --- and measures how the
fitted choice weights attenuate and how the recovered $\bcoef$ degrades
while the prediction metrics move. The hypothesis, from
\cref{prop:ch08:attenuation} and \cref{cor:ch08:pooled}, is that
structure erodes before prediction does: attenuation should hit
\emph{interpretation} (structural readings of $\bcoef$, cross-regime
transfer) long before it hits ranking or likelihood. Stage two makes the
noise informative: the loader emits drifting covariates with LOCF ages,
and ranking bias is reported by staleness decile. This stage measures
the differential distortion of \cref{prop:ch08:ranking}, which
uninformative noise cannot see, and doubles as the standing harness for
Protocol~\ref{prot:ch08:gaptest}.

Because aggregate metrics are expected to dilute exactly the damage this
chapter is about --- fitted structure decaying while headline prediction
numbers barely move --- the chapter installs a standing
...
each rung consumes the previous rung's outputs, and the working
hypothesis --- to be tested in \cref{ch:results-synth} --- is that
prediction is initially robust to staleness, so the
expensive rungs must prove their marginal value on a measured gap, not
...
extends the planned covariate-noise campaign with LOCF-age worlds and
