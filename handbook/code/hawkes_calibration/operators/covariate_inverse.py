...
def _pc_paths(n, N, p, n_switch, rng, lo=-1.0, hi=1.0):
    ...
    for b in range(n):
        for c in range(p):
            edges = (
                np.sort(rng.integers(1, N, size=n_switch - 1))
                if n_switch > 1
                else np.array([], int)
            )
...
def sample_dataset(
    ...
    return dict(
        s=s, A=A, delta=delta, delta_vec=design.pack_delta(delta), Z=Z, xi=xi, t=t, theta=theta
    )
...
def batched_forward_numpy(s, A, delta, Z, t_grid, theta=1.0):
    ...
    for b in range(n):
        out[b] = solve_mbpp_ltv(
            lambda tt, ss=s[b]: ss, A[b], B, t, modulation=_modulation_callable(Z[b], delta[b], t)
        )
    return out
...
# ===========================================================================
def recover_params(
    design, data, theta=1.0, sigma=0.0, n_obs=1, steps=6000, lr=3e-2, early_stop=True, seed=0
):
    s = jnp.asarray(data["s"])
    Zf = jnp.asarray(fine_covariates(data["Z"], data["t"]))
    dts = jnp.asarray(np.diff(data["t"]))
    mask = jnp.asarray(design.mask.astype(float))
    n, M = data["s"].shape
    ...
    if sigma > 0:
        obs = np.mean(
            [
                np.maximum(xi * (1.0 + sigma * rng.standard_normal(xi.shape)), 0.0)
    ...
    for i in range(steps):  # noqa: B007 -- `i` is read after the loop (steps_used)
        l, gr = g(par)
        up, st = opt.update(gr, st)
        par = optax.apply_updates(par, up)
        if early_stop and floor > 0 and float(l) <= 1.05 * floor:
            break
    A, dl = par
    Ah = np.asarray(A)
    dfull = np.asarray(dl) * design.mask[None]
    dvech = dfull[:, design.mask]
    A_rel = np.linalg.norm((Ah - data["A"]).reshape(n, -1), axis=1) / (
        np.linalg.norm(data["A"].reshape(n, -1), axis=1) + 1e-12
    )
    d_rel = np.linalg.norm(dvech - data["delta_vec"], axis=1) / (
        np.linalg.norm(data["delta_vec"], axis=1) + 1e-12
    )
    return dict(
        A=Ah,
        delta_vec=dvech,
        steps_used=i + 1,
        final_loss=float(l),
        A_rel_mean=float(A_rel.mean()),
        A_rel_median=float(np.median(A_rel)),
        delta_rel_mean=float(d_rel.mean()),
        delta_rel_median=float(np.median(d_rel)),
        A_rel=A_rel,
        d_rel=d_rel,
    )
...
def _conv1d(x, W, b):
    y = jax.lax.conv_general_dilated(x, W, (1,), "SAME", dimension_numbers=("NWC", "WIO", "NWC"))
    return y + b
...
class AmortizedCovariateInverse:
    ...
    def _init(self, key):
        ...
        for _ in range(self.depth):
            ...
            W = jax.random.normal(k, (self.kernel, cin, self.width)) * np.sqrt(
                2.0 / (self.kernel * cin)
            )
        ...
        for a, b in zip(sizes[:-1], sizes[1:]):
            key, k = jax.random.split(key)
            ps["dense"].append(
                (jax.random.normal(k, (a, b)) * np.sqrt(2.0 / (a + b)), jnp.zeros(b))
            )
        return ps
    ...
    # -- training -----------------------------------------------------------
    def train(
        self, data, epochs=4000, lr=2e-3, batch=128, sigma_train=0.1, val=None, log_every=0, seed=0
    ):
        Z = np.asarray(data["Z"])
        xi = np.asarray(data["xi"])
        Y = np.concatenate([data["A"].reshape(len(Z), -1), data["delta_vec"]], axis=1)
        self.x_mean = np.concatenate(
            [xi.reshape(-1, self.M).mean(0), Z.reshape(-1, self.design.p).mean(0)]
        )
        self.x_std = (
            np.concatenate([xi.reshape(-1, self.M).std(0), Z.reshape(-1, self.design.p).std(0)])
            + 1e-6
        )
        ...
        xm = jnp.asarray(self.x_mean)
        xs = jnp.asarray(self.x_std)
        n = len(Z)
        ...
        for ep in range(epochs):
            ...
            if val is not None and (ep % max(1, epochs // 40) == 0 or ep == epochs - 1):
                ...
                if log_every:
                    print(
                        f"  epoch {ep:5d}  loss={float(loss):.4e}  "
                        f"val A-rel={v:.4f} (best {best:.4f})",
                        flush=True,
                    )
    ...
    def predict(self, Z, xi):
        Z = np.atleast_3d(np.asarray(Z, float))
    ...
    def eval_param_error(self, data, sigma=0.0, seed=123):
        Z = np.asarray(data["Z"])
        xi = np.asarray(data["xi"]).copy()
        if sigma > 0:
            rng = np.random.default_rng(seed)
            xi = np.maximum(xi * (1.0 + sigma * rng.standard_normal(xi.shape)), 0.0)
        Ah, dvech = self.predict(Z, xi)
        A, dvec = data["A"], data["delta_vec"]
        A_rel = np.linalg.norm((Ah - A).reshape(len(A), -1), axis=1) / (
            np.linalg.norm(A.reshape(len(A), -1), axis=1) + 1e-12
        )
        ...
        xi_true = data["xi"]
        rec = np.linalg.norm((xi_rec - xi_true).reshape(len(A), -1), axis=1) / (
            np.linalg.norm(xi_true.reshape(len(A), -1), axis=1) + 1e-12
        )
        return dict(
            A_rel_mean=float(A_rel.mean()),
            A_rel_median=float(np.median(A_rel)),
            delta_rel_mean=float(d_rel.mean()),
            delta_rel_median=float(np.median(d_rel)),
            recon_rel_mean=float(rec.mean()),
            recon_rel_median=float(np.median(rec)),
        )
