...
from hawkes_calibration import (
    simulate_synthetic_startup_market, fit_startup_survival, evaluate_survival,
    survival_predict_proba, make_tracked_mask,
)
from hawkes_calibration.sector_survival import _prepare, _objective, sector_week_counts
...
def test_objective_gradient_matches_finite_differences():
    data = _data(); tracked = _tracked(data)
    Z = data.startup_features.astype(float); M = 4; p = Z.shape[2]
    secw = sector_week_counts(data.startup_counts, data.startup_sector, M)
    prep = _prepare(data.events, Z, data.startup_sector, data.active.astype(bool),
                    data.startup_counts.astype(float), tracked, secw, 50, 8, 8)
    dims = (p, M, 2); n = p + M * p + M + 1 + 2
    ...
    for i in range(n):
        ...
        gn[i] = (_objective(a, prep, dims, 1e-3, 1e-2, 1e-2)[0]
                 - _objective(b, prep, dims, 1e-3, 1e-2, 1e-2)[0]) / (2 * e)
...
def test_fit_respects_cooldown_sign_and_probabilities_normalise():
    data = _data(); tracked = _tracked(data)
    fit = fit_startup_survival(data.events, data.startup_features, data.startup_sector,
                               data.active, data.startup_counts, tracked=tracked,
                               train_end=55, cooldown_weeks=8, max_iter=120)
    assert np.all(fit.cooldown_coef <= 1e-9)         # self-inhibition (recency)
    cand, pr, p_out = survival_predict_proba(
        fit, data.startup_features, data.startup_sector, data.active,
        data.startup_counts, week=60, sector=0, tracked=tracked)
...
def test_outside_option_discriminates_and_ranks_above_chance():
    ...
    fit = fit_startup_survival(data.events, data.startup_features, data.startup_sector,
                               data.active, data.startup_counts, tracked=tracked,
                               train_end=55, cooldown_weeks=8, max_iter=120)
    ev = evaluate_survival(fit, data.events, data.startup_features, data.startup_sector,
                           data.active, data.startup_counts, tracked=tracked,
                           start_week=55, end_week=80)
