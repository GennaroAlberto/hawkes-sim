...
def test_gradients_match_finite_differences_both_links():
    d = _market()
    Z = d.startup_features.astype(float)
    M = 6
    p = Z.shape[2]
    ...
    for link in ("linear", "exp"):
        ...
        for i in range(n):
            ...
            b[i] -= e
            gn[i] = (
                _objective(a, prep, p, M, link, 1e-2)[0] - _objective(b, prep, p, M, link, 1e-2)[0]
            ) / (2 * e)
