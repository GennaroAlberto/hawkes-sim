...
def test_delta_zero_reduces_to_multivariate_ode():
    ...
    def s(tt):
        return np.array([2.0, 1.5, 1.8])

    xi_ode = solve_mbpp_ode_multivariate(s, A, B, t)
    xi_ltv = solve_mbpp_ltv(
        s, A, B, t, modulation=_multi_modulation(lambda tt: np.array([0.0]), np.array([0.0]), M)
    )
...
def test_multivariate_fitter_recovers_baseline_and_delta_sign():
    ...
    bks = np.linspace(0, T, 6)
    pop = rng.normal(0, 0.7, 5)
    pop -= pop.mean()
    ...
    _, Xi = solve_mbpp_ltv(
        ...
        theta * np.ones((M, M)),
        fine,
        modulation=_multi_modulation(Z, delta, M),
