...
def _market(seed=4):
    ...
    A = np.zeros((M, M))
    A[np.arange(M), np.arange(M)] = np.array([0.3, 0.5, 0.7, 0.45])
...
def test_analytic_gradient_matches_finite_differences():
    ...
    for i in range(n):
        ...
        b[i] -= e
        gn[i] = (
            _nll_and_grad(a, prep, M, p, 2e-3, mf)[0] - _nll_and_grad(b, prep, M, p, 2e-3, mf)[0]
        ) / (2 * e)
...
def test_fit_recovers_baselines_covariates_and_self_inhibition():
    ...
    fit = fit_block_hawkes(
        data, n_grid=420, w_self=4.0, w_cross=0.6, mask=mask, l2=2e-3, max_iter=300
    )
...
def test_mask_zeroes_disallowed_excitation():
    ...
    fit = fit_block_hawkes(
        data, n_grid=300, w_self=4.0, w_cross=0.6, mask=mask, l2=2e-3, max_iter=80
    )
