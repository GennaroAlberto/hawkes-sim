...
def test_two_loglik_implementations_agree():
    ...
    for _trial in range(8):
        ...
        events = simulate_multivariate_hawkes(
            gamma0, alpha, beta, T=300.0, seed=int(rng.integers(1e9))
        )
...
def _fd_grad_gamma0_alpha(events, T, gamma0, alpha, beta, gamma=None, cov=None, eps=1e-6):
    ...
    for m in range(M):
        ...
        gm[m] -= eps
        g_g0[m] = (
            log_likelihood(events, T, gp, alpha, beta, gamma, cov)
            - log_likelihood(events, T, gm, alpha, beta, gamma, cov)
        ) / (2 * eps)
    g_al = np.zeros((M, M))
    for m in range(M):
        for j in range(M):
            ...
            am[m, j] -= eps
            g_al[m, j] = (
                log_likelihood(events, T, gamma0, ap, beta, gamma, cov)
                - log_likelihood(events, T, gamma0, am, beta, gamma, cov)
            ) / (2 * eps)
    return g_g0, g_al
...
def test_analytic_gradient_matches_fd():
    ...
    M = 3
    alpha = np.array([[0.20, 0.10, 0.05], [0.00, 0.25, 0.10], [0.08, 0.00, 0.18]])
...
def test_analytic_gradient_with_covariates():
    ...
    events = simulate_multivariate_hawkes(
        gamma0, alpha, beta, T=T, gamma=gamma, covariate=cov, seed=3
    )
    ...
    for m in range(2):
        for k in range(gamma.shape[1]):
            ...
            gm[m, k] -= eps
            fgam[m, k] = (
                log_likelihood(events, T, gamma0, alpha, beta, gp, cov)
                - log_likelihood(events, T, gamma0, alpha, beta, gm, cov)
            ) / (2 * eps)
    d0 = np.max(np.abs(ag0 - fg0))
    da = np.max(np.abs(aalpha - falpha))
    dg = np.max(np.abs(agam - fgam))
...
def test_univariate_mle_large_T():
    ...
    for _ in range(6):
        ev = simulate_multivariate_hawkes(
            [np.log(mu)], [[alpha]], [[beta]], T=T, seed=int(rng.integers(1e9))
        )
        res = fit_univariate(ev[0], T, beta=beta, se=True)
        mu_hat.append(np.exp(res.gamma0[0]))
        al_hat.append(res.alpha[0, 0])
    ...
    print(
        f"  T={T:.0f}  <N>={np.mean(n_ev):.0f}  mu_hat={mu_hat.mean():.3f}±{mu_hat.std():.3f}"
...
def test_multivariate_mle_M6():
    ...
    print(
        f"  M={M} T={T:.0f} N={sum(e.size for e in ev)} fit {dt:.1f}s | "
...
def test_near_critical_stability():
    ...
    res = fit_univariate(ev[0], T, beta=beta, se=True)
    print(
        f"  near-critical alpha/beta=0.85  N={N}  rate emp/theo={N / T:.3f}/{rate_theo:.3f}  "
...
def test_lasso_highdim_M10():
    ...
    print(
        f"  M={M} T={T:.0f} N={N} | MLE prec/rec={p_mle:.2f}/{r_mle:.2f} zMAE={zmae_mle:.3f}"
...
def test_covariate_event_time_recovery():
    ...
    ev = simulate_multivariate_hawkes(gamma0, alpha, beta, T=T, gamma=gamma, covariate=cov, seed=11)
    res = fit_multivariate_with_covariates(ev, T, beta=beta, covariate=cov, se=True)
    print(
        f"  N={sum(e.size for e in ev)}  gamma_hat={res.gamma.ravel()}  (true {gamma.ravel()})"
...
def test_ic_large_scale():
    ...
    res = fit_mbpp_ic_multi(obs, counts_list, exo_list, loss="ic-ll", endogenous=True, n_restarts=3)
    print(
        f"  {len(counts_list)} seqs, {obs.size - 1} intervals | kappa_hat={res.kappa:.3f}"
...
def test_operator_multivariate_M15():
    ...
    print(
        f"  M={M} spectral radius={np.max(np.abs(np.linalg.eigvals(G))):.3f} | "
