...
def test_mcmc_recovers_kappa_and_is_honest_about_theta():
    ...
    res = fit_mbpp_bayes(
        obs, counts, Z, endogenous=True, method="mcmc", n_samples=2500, burn=800, n_chains=2, seed=0
    )
...
def test_laplace_agrees_with_mcmc_on_kappa():
    ...
    lap = fit_mbpp_bayes(obs, counts, Z, endogenous=True, method="laplace", n_samples=3000, seed=0)
...
def test_sampler_targets_a_gaussian():
    ...
    def log_post(x):
        return -0.5 * (x - mu) @ P @ (x - mu)

    chains, acc = adaptive_metropolis(
        log_post, np.zeros(2), n_samples=6000, burn=2000, n_chains=2, seed=0
    )
