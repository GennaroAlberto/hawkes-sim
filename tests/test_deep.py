...
def test_volterra_matches_closed_form_convolution():
    ...
    def K(tt, uu):
        return kappa * theta * np.exp(-theta * (tt - uu))
...
def test_volterra_matches_ltv_excitation():
    ...
    def Kx(tt, uu):
        return kappa * theta * float(mod(tt)[0, 0]) * np.exp(-theta * (tt - uu))
...
def test_sumexp_closed_form_compensator():
    mu = 1.5
    thetas = np.array([2.0, 0.4])
    kappas = np.array([0.15, 0.45])
    a = kappas * thetas
...
def test_gof_diagnostics():
    ...
    lam = 3.0
    T = 300.0
    ev = np.cumsum(rng.exponential(1 / lam, int(lam * T * 1.2)))
    ev = ev[ev < T]
