...
def simulate_synthetic_startup_market(
    *,
    ...
    Signal-strength knobs (defaults reproduce the original DGP):

    * ``base_log_rate`` -- mean of the sector log-intercept; raise it for a higher
      (but still moderate) event volume so the count signal is learnable.
    * ``covariate_strength`` -- multiplies the macro-covariate coefficients, i.e. how
      much the exogenous covariates move the sector rates.
    * ``excitation_radius`` -- if set, the true lagged excitation matrix is rescaled
      to have exactly this aggregate spectral radius (kept well below 1 for a stable,
      noncritical process).
    """
