...
from .pino import conv_matrix, sample_instances, exact_solution  # reuse the problem def
...
class JAXMultivariateMBPPOperator:
    def __init__(self, M, t_grid, theta=1.0, p=64, hidden=160, depth=3,
                 n_fourier=16, seed=0):
        self.M = int(M)
        self._seed = int(seed)
    ...
    def _params_vec(self, S, A):
        P = np.concatenate([np.asarray(S, float),
                            np.asarray(A, float).reshape(len(A), -1)], axis=1)
        return (P - self.pin_mean) / self.pin_std

    # -- training -----------------------------------------------------------
    def train(self, S, A, epochs=8000, lr=2e-3, batch=128, anchors=None,
              w_res=1.0, w_anchor=1.0, w_ic=1.0, seed=0, val=None, log_every=0):
        S = np.asarray(S, float); A = np.asarray(A, float)
        Praw = np.concatenate([S, A.reshape(len(A), -1)], axis=1)
        self.pin_mean = Praw.mean(0); self.pin_std = Praw.std(0) + 1e-8
        self.yscale = float(S.mean() / 0.7)

        P = jnp.asarray(self._params_vec(S, A))
        Sj = jnp.asarray(S); Aj = jnp.asarray(A)
        n = len(S)
        ...
        if has_anchor:
            ...
            Saj = jnp.asarray(Sa); Aaj = jnp.asarray(Aa); XIaj = jnp.asarray(XIa)
            na = len(Sa)
        ...
        def loss_fn(params, pb, sb, ab, pa, sa, aa, xia):
            ...
            if has_anchor:
                ...
                rel_a = jnp.mean(jnp.sum((xa - xia) ** 2, (1, 2)) / (jnp.sum(xia ** 2, (1, 2)) + 1e-6))
        ...
        for ep in range(epochs):
            ...
            if val is not None and (ep % val_every == 0 or ep == epochs - 1):
                ...
                hist["epoch"].append(ep); hist["loss"].append(float(loss)); hist["val_rel_l2"].append(rel)
                if rel < best_val:
                    best_val = rel
                    best_params = jax.tree_util.tree_map(lambda x: x, params)
                if log_every:
                    print(f"  epoch {ep:5d}  loss={float(loss):.3e}  val rel-L2={rel:.4f}"
    ...
    def save(self, path):
        ...
        for i, (W, b) in enumerate(self.branch):
            flat[f"bW{i}"] = np.asarray(W); flat[f"bb{i}"] = np.asarray(b)
        for i, (W, b) in enumerate(self.trunk):
            flat[f"tW{i}"] = np.asarray(W); flat[f"tb{i}"] = np.asarray(b)
        np.savez(path, b0=np.asarray(self.b0), pin_mean=self.pin_mean, pin_std=self.pin_std,
                 yscale=self.yscale, t=self.t, theta=self.theta, p=self.p, M=self.M,
                 n_fourier=(self.tq.shape[1] - 1) // 2, seed_used=self._seed,
                 n_branch=len(self.branch), n_trunk=len(self.trunk), **flat)

    @classmethod
    def load(cls, path):
        d = np.load(path, allow_pickle=False)
        op = cls(int(d["M"]), d["t"], theta=float(d["theta"]), p=int(d["p"]),
                 n_fourier=int(d["n_fourier"]), seed=int(d["seed_used"]))
        op.branch = [(jnp.asarray(d[f"bW{i}"]), jnp.asarray(d[f"bb{i}"])) for i in range(int(d["n_branch"]))]
        op.trunk = [(jnp.asarray(d[f"tW{i}"]), jnp.asarray(d[f"tb{i}"])) for i in range(int(d["n_trunk"]))]
        op.b0 = jnp.asarray(d["b0"])
        op.pin_mean = d["pin_mean"]; op.pin_std = d["pin_std"]; op.yscale = float(d["yscale"])
        return op
